...
const CONTAINER_PADDING = 24; // styles.container padding 12 * 2
const CHART_WIDTH = SCREEN_WIDTH - CONTAINER_PADDING;
...
const BarChartCard = ({
  ...
  const [localSelectedKey, setLocalSelectedKey] = useState(
    granularity === 'daily' ? currentDayKey : currentMonthKey,
  );
  ...
  const barWidth = 30;
  const barMargin =
    granularity === 'daily' || filterType?.includes('week') ? 6 : 14;
  const fullColumnWidth = barWidth + barMargin * 2;

  useEffect(() => {
    setLocalSelectedKey(
      granularity === 'daily' || filterType?.includes('week')
        ? currentDayKey
        : currentMonthKey,
    );
  ...
  const processedData = useMemo(() => {
    ...
    if (isDaily) {
      ...
      rawExpenses.forEach(exp => {
        if (!exp.month) return;
        monthTotals[exp.month] =
          (monthTotals[exp.month] || 0) + (exp.amountRupees || 0);
      });
      let startYear, endYear, startMonth, endMonth;
      if (filterType === 'yearly' && filterValue) {
        startYear = parseInt(filterValue);
        endYear = startYear;
        startMonth = 0;
        ...
        if (sortedKeys.length > 0) {
          [startYear, startMonth] = sortedKeys[0].split('-').map(Number);
          startMonth -= 1;
        } else {
      ...
      while (
        ...
        const isRecent =
          new Date(now.getFullYear(), now.getMonth() - 6, 1) <= d;
        if (
          monthTotals[k] ||
          isRecent ||
          (currYear === now.getFullYear() && currMonth === now.getMonth())
        ) {
          items.push({
            label: MONTH_NAMES[d.getMonth()],
            fullLabel: `${FULL_MONTH_NAMES[d.getMonth()]} '${String(
              d.getFullYear(),
            ).slice(2)}`,
            key: k,
          });
      ...
      if (items.length > 12 && filterType === 'all_time')
        items.splice(0, items.length - 12);
    }
    const totals = {};
    const incomeTotals = {};
    ...
    // Only filter for "all_time" to avoid huge empty ranges,
    // but keep specific ranges (week, month, year) intact so they don't shrink.
    const finalItems =
      filterType === 'all_time'
        ? items.filter(
            item =>
              (totals[item.key] || 0) > 0 || (incomeTotals[item.key] || 0) > 0,
          )
        : items;

    const values = finalItems.map(item => totals[item.key] || 0);
  ...
  const activeIdx = processedData.items.findIndex(
    item => item.key === localSelectedKey,
  );
  const displayIdx =
    activeIdx >= 0 ? activeIdx : processedData.items.length - 1;
  ...
  const periodBaseTotal = useMemo(() => {
    ...
    if (granularity === 'daily' || filterType?.includes('week')) {
      ...
      return Object.values(processedData.refTotals).reduce(
        (sum, v) => sum + v,
        0,
      );
    }
  ...
  useEffect(() => {
    const task = InteractionManager.runAfterInteractions(() => {
      processedData.items.forEach((item, idx) => {
        ...
        const heightPercent =
          val === 0 ? 0 : Math.max((val / (processedData.max * 1.15)) * 100, 4);
  ...
  useEffect(() => {
    const task = InteractionManager.runAfterInteractions(() => {
      categories.forEach((cat, idx) => {
        const percent =
          periodBaseTotal > 0 ? (cat.totalMoneyOut / periodBaseTotal) * 100 : 0;
  ...
  const shouldStretch = filterType === 'yearly' || filterType?.includes('week');
  ...
  const styles = useMemo(
    () =>
      StyleSheet.create({
        ...
        monthText: { fontSize: 15, fontFamily: Fonts.medium, marginBottom: 4 },
        amountText: { fontSize: 40, fontFamily: Fonts.bold },
        chartArea: { height: 200, marginBottom: 10 },
        barsScrollContent: {
          flexGrow: 1,
          paddingHorizontal: 4,
        },
        barsContainer: {
          ...
          justifyContent: shouldStretch ? 'space-around' : 'flex-start',
        },
        avgLine: {
          position: 'absolute',
          left: 0,
    ...
    >
      <View style={styles.header}>
        <View style={{ flex: 1 }}>
          ...
          <Text
            style={[styles.avgHeaderLabel, { color: theme.colors.secondary }]}
          >
            Average:{' '}
            {formatAmount(processedData.avg || 0, user?.currency, {
              compact: true,
            })}
          </Text>
        </View>
      ...
      <View style={styles.statsContainer}>
        ...
        >
          {displayItem?.fullLabel}
        </Text>
        <Text style={[styles.amountText, { color: theme.colors.text }]}>
          {isPrivacyMode
            ? '******'
            : formatAmount(displayAmount || 0, user?.currency)}
        </Text>
      </View>
          ...
          <View style={styles.barsContainer}>
            <View
              style={[
                ...
                {
                  bottom:
                    50 + (processedData.avg / (processedData.max * 1.15)) * 130,
                },
              ]}
            >
              <View
              ...
              return (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    {
                      width: barWidth,
                      marginHorizontal: shouldStretch ? 0 : barMargin,
                    },
                  ]}
                >
                  <View
                    style={[
                      styles.barTrack,
                      { width: barWidth, backgroundColor: 'transparent', overflow: 'visible' },
                    ]}
                  >
                    <Animated.Text
                      style={[
      ...
      <View style={styles.categoriesSection}>
        ...
        {categories.length > 0 ? (
          categories.map((cat, index) => {
            ...
            const percent =
              periodBaseTotal > 0
                ? (cat.totalMoneyOut / periodBaseTotal) * 100
                : 0;
              ...
              >
                ...
                <View style={styles.categoryInfo}>
                  <View style={styles.categoryHeader}>
                    ...
                    >
                      {formatAmount(cat.totalMoneyOut, user?.currency)}
                    </Text>
                  </View>
                  {activeType === 'expense' && (
